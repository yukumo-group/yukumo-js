...
export function parsePE(buffer: ArrayBuffer): PEResult {
  ...
  if (exportDirRVA !== 0) {
    ...
    for (let i = 0; i < numNames; i++) {
      ...
      exports[name] = view.getUint32(functionsOffset + ordinal * 4, true);
    }
  }
...
/**
 * Map a PE32 file into a virtual image (headers + sections at RVAs).
 * Needed when FileAlignment != SectionAlignment.
 */
export function mapPEImage(buffer: ArrayBuffer): Uint8Array {
  const view = new DataView(buffer);
  const uint8 = new Uint8Array(buffer);
  const peOffset = view.getUint32(0x3c, true);
  const optionalHeaderOffset = peOffset + 24;
  ...
  const sizeOfImage = view.getUint32(optionalHeaderOffset + 56, true);

  const image = new Uint8Array(sizeOfImage);
  image.set(uint8.subarray(0, Math.min(sizeOfHeaders, uint8.length, sizeOfImage)));
  ...
  for (let i = 0; i < numberOfSections; i++) {
    ...
    if (pointerToRawData === 0 || virtualAddress >= sizeOfImage) {
      continue;
    }
    const copySize = Math.min(
      sizeOfRawData,
      virtualSize || sizeOfRawData,
      uint8.length - pointerToRawData,
      sizeOfImage - virtualAddress
    );
    if (copySize > 0) {
      image.set(
        uint8.subarray(pointerToRawData, pointerToRawData + copySize),
        virtualAddress
      );
    }
  }
